...
Subscribes to /orb_slam/point_cloud (sensor_msgs/PointCloud2) and renders the
points in a live window.  Open3D is used when available (best quality); the
node falls back to a matplotlib 3-D scatter plot otherwise.

The Open3D event loop and the matplotlib draw loop both require the main
thread, so rospy.spin() is dispatched to a daemon thread.

ROS parameters
~~~~~~~~~~~~~~
~point_cloud_topic : input topic  (default: /orb_slam/point_cloud)
~max_display_pts   : subsample cap for display  (default: 8000)
...
# ---------------------------------------------------------------------------
# Backend selection — prefer Open3D, fall back to matplotlib
# ---------------------------------------------------------------------------
try:
...
class _Open3DVisualizer:
    def __init__(self, max_pts: int, point_size: float):
        ...
        self._pts: np.ndarray = np.empty((0, 3), dtype=np.float64)
        self._updated = False
